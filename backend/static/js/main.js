...
socket.on('connect', function() {
  if (typeof eid !== 'undefined') {
    ...
    socket.emit('join', { eid: eid });
  }
});

// Handle successful join
socket.on('joined', function(data) {
  console.log(`Successfully joined room ${data.room}. Members: ${data.member_count}`);
  updateRoomStatus(data.member_count);
});

// Handle when another user joins
socket.on('user-joined', function(data) {
  console.log(`Another user joined. Members: ${data.member_count}`);
  updateRoomStatus(data.member_count);
});

// Handle when another user leaves
socket.on('user-left', function(data) {
  console.log(`A user left. Members: ${data.member_count}`);
  updateRoomStatus(data.member_count);
  
  // Clean up connection if we're alone
  if (data.member_count === 1) {
    console.log('Other peer disconnected');
    // Could optionally show a "waiting for peer" message
  }
});
...
// Helper function to update UI based on room status
function updateRoomStatus(memberCount) {
  ...
  if (memberCount === 1) {
    ...
    console.log('Meeting is ready with 2 participants');
  }
}
...
// ICE server configuration with multiple STUN servers
const configuration = {
  iceServers: [
